...
class FilterInput : public IInput {
  ...
  void setMetadata(Metadata) override { assert(false); }

  // IInput
  int isConnected() const override { return delegate->isConnected(); }
  void connect() override { delegate->connect(); }
  void disconnect() override { delegate->disconnect(); }
  Metadata getMetadata() const override { return delegate->getMetadata(); }
  bool updateMetadata(Data &data) override { return delegate->updateMetadata(data); }
