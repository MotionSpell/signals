...
std::unique_ptr<Pipeline> createPipelineFromJSON(const std::string &jsonText,
  ...
  if(version != JSON_VERSION) {
    auto const err =
          std::string("Config version is") + std::to_string(version) + ", expected " + std::to_string(JSON_VERSION);
    throw std::runtime_error(err.c_str());
  }
  ...
  if(json.has("connections")) {
    ...
    for(auto const &conn : connections.arrayValue) {
      ...
      for(auto const &prop : srcConn.objectValue) {
        ...
        if(!src.mod) {
          auto const err =
                std::string("Connection " + std::to_string(connCount) + ": source module \"" + prop.key + "\" unknown");
          throw std::runtime_error(err.c_str());
        }
      ...
      for(auto const &dstConn : dstConns.arrayValue) {
        for(auto const &prop : dstConn.objectValue) {
          ...
          if(!dst.mod) {
            auto const err = std::string(
                  "Connection " + std::to_string(connCount) + ": destination module \"" + prop.key + "\" unknown");
            throw std::runtime_error(err.c_str());
          }
