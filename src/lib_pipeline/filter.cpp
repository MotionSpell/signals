...
Filter::~Filter() { log(Info, "Pipeline: destroy"); }
...
void Filter::activate(bool enable) { active = enable; }

void Filter::setDelegate(std::shared_ptr<IModule> module) { delegate = module; }

int Filter::getNumInputs() const { return delegate->getNumInputs(); }

int Filter::getNumOutputs() const { return delegate->getNumOutputs(); }
...
Metadata Filter::getOutputMetadata(int i) { return getOutput(i)->getMetadata(); }

/* source modules are stopped manually - then the message propagates to other connected modules */
bool Filter::isSource() { return active; }
...
bool Filter::exception(std::exception_ptr eptr) { return m_eventSink->exception(eptr); }
