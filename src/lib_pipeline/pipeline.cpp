...
struct StatsRegistry : IStatsRegistry {
  StatsRegistry()
      : shmem(createSharedMemory(size, std::to_string(getPid()).c_str(), true))
      , entryIdx(0) {
  ...
  StatsEntry *getNewEntry(const char *name) override {
    ...
    if(entryIdx >= maxNumEntry)
      throw std::runtime_error(
            format("SharedMemory: accessing too far (%s with max=%s).", entryIdx - 1, maxNumEntry - 1));
...
IFilter *Pipeline::add(char const *type, const void *va) {
  ...
  auto createModule = [&](Modules::KHost *host) { return loadModule(type, host, va); };

  return addModuleInternal(name, createModule);
}

void Pipeline::removeModule(IFilter *module) {
  auto findIf = [module](Pipelines::Graph::Connection const &c) { return c.src.id == module || c.dst.id == module; };
  ...
  auto removeIt = [module](std::unique_ptr<Filter> const &m) { return m.get() == module; };
  ...
  auto i_node = std::find_if(
        graph->nodes.begin(), graph->nodes.end(), [module](Pipelines::Graph::Node const &n) { return n.id == module; });
...
void Pipeline::connect(OutputPin prev, InputPin next, bool inputAcceptMultipleConnections) {
  ...
  graph->connections.push_back(
        Graph::Connection{graph->nodeFromId(prev.mod), prev.index, graph->nodeFromId(next.mod), next.index});
}
...
void Pipeline::waitForEndOfStream() {
  ...
  while(remainingNotifications > 0) {
    m_log->log(Debug,
          format("Pipeline: condition (remaining: %s) (%s modules in the pipeline)", remainingNotifications,
                modules.size())
                .c_str());
...
void Pipeline::registerErrorCallback(std::function<bool(const char *)> cbk) { errorCbk = cbk; }
