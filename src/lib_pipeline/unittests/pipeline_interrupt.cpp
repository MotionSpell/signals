...
struct FakeTransformer : public Modules::ModuleS {
  FakeTransformer(Modules::KHost *) { out = addOutput(); }
  void processOne(Modules::Data) override { out->post(out->allocData<DataRaw>(1)); }
...
unittest("pipeline: destroy while running: fast producer, slow consumer") {
  struct SlowSink : public Modules::ModuleS {
    ...
    void processOne(Modules::Data) override { std::this_thread::sleep_for(20ms); }
  };
