...
struct BoxBrowser {
  ...
  uint32_t fourcc() const { return (data[4] << 24) | (data[5] << 16) | (data[6] << 8) | (data[7] << 0); }
  size_t size() const { return (data[0] << 24) | (data[1] << 16) | (data[2] << 8) | (data[3] << 0); }
  SpanC contents() const { return {data.ptr + 8, size() - 8}; }
  ...
  bool hasSibling() const { return data.len > size(); }
...
struct Fmp4Splitter : ModuleS {
  ...
  void processOne(Data data) override { m_separator.process(data); }
  ...
  void ensureConverter() {
    ...
    case FOURCC("mp4a"): {
      ...
      auto deliver = [this](Data data) { output->post(data); };
  ...
  std::shared_ptr<MetadataPkt> createMetadata() {
    ...
    default: {
      char msg[256];
      snprintf(msg, sizeof msg, "Unknown SampleEntry: %c%c%c%c", (m_codecFourcc >> 24) & 0xff,
            (m_codecFourcc >> 16) & 0xff, (m_codecFourcc >> 8) & 0xff, (m_codecFourcc >> 0) & 0xff);
      ...
      meta = make_shared<MetadataPkt>(VIDEO_PKT);
      snprintf(msg, sizeof msg, "%c%c%c%c", (m_codecFourcc >> 24) & 0xff, (m_codecFourcc >> 16) & 0xff,
            (m_codecFourcc >> 8) & 0xff, (m_codecFourcc >> 0) & 0xff);
      meta->codec = msg;
    }
    }
  ...
  void processDecoderSpecificInfoDescriptor(SpanC &s) { m_codecSpecificInfo.assign(s.ptr, s.ptr + s.len); }
