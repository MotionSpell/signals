...
SDL_AudioSpec toSdlAudioSpec(PcmFormat cfg) {
  ...
  case F32:
    ...
    break;
  default:
    throw std::runtime_error("Unknown PcmFormat sample format");
  }
  return audioSpec;
}
...
PcmFormat toPcmFormat(SDL_AudioSpec audioSpec) {
  ...
  case AUDIO_F32:
    ...
    break;
  default:
    throw std::runtime_error("Unknown SDL audio sample format");
  }
  return fmt;
}
...
struct SDLAudio : ModuleS {
  SDLAudio(KHost *host, IClock *clock)
    ...
    auto pushAudio = [this](Data d) { push(d); };
  ...
  void fillAudio(Span buffer) {
    ...
    auto const relativeTimePositionIn180k = std::min<int64_t>(
          m_fifoTime - bufferTimeIn180k, IClock::Rate * 10); // clamp to 10s to avoid integer overflows below
    ...
    if(relativeTimePositionIn180k < -TOLERANCE) {
      ...
      m_host->log(Warning,
            format("must drop fifo data (%sms) (delta=%ss)", numSamplesToDrop * 1000.0f / m_outputFormat.sampleRate,
                  (double)relativeTimePositionIn180k / IClock::Rate)
                  .c_str());
      ...
      m_host->log(Warning,
            format("insert silence (%sms) (delta=%ss)", numSilenceSamples * 1000.0f / m_outputFormat.sampleRate,
                  (double)relativeTimePositionIn180k / IClock::Rate)
                  .c_str());
  ...
  uint64_t fifoSamplesToRead() const { return m_fifo.bytesToRead() / m_outputFormat.getBytesPerSample(); }
