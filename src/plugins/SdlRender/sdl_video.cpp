...
struct SDLVideo : ModuleS {
  ...
  void doRender() {
    ...
    window = SDL_CreateWindow("Signals SDLVideo renderer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
          pictureFormat.res.width, pictureFormat.res.height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if(!window)
  ...
  void displayFrame(Data data) {
    ...
    if(pictureFormat.format == PixelFormat::I420) {
      SDL_UpdateYUVTexture(texture, nullptr, pic->getPlane(0), (int)pic->getStride(0), pic->getPlane(1),
            (int)pic->getStride(1), pic->getPlane(2), (int)pic->getStride(2));
    } else {
  ...
  void createTexture() {
    ...
    texture = SDL_CreateTexture(renderer, pixelFormat2SDLFormat(pictureFormat.format), SDL_TEXTUREACCESS_STATIC,
          pictureFormat.res.width, pictureFormat.res.height);
    if(!texture)
