...
struct RegulatorMulti : public ModuleDynI {
  ...
  void flush() override {
    dispatch([](Rec const &) { return true; });
  }
  ...
  void process() override {
    ...
    if(data->getMetadata())
      if(data->getMetadata()->isAudio() || data->getMetadata()->isVideo()) {
        auto const newMediaDispatchTime =
              std::max<int64_t>(mediaDispatchTime, data->get<DecodingTime>().time - maxMediaTimeDelay);
  ...
  void reset() { mediaDispatchTime = std::numeric_limits<int64_t>::min(); }
