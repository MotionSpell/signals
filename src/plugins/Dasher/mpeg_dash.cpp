...
std::string formatDateFolder(int64_t timestamp) {
  ...
  char buffer[256];
  sprintf(buffer, "%04d_%02d_%02d_%02d_%02d_%02d", 1900 + date.tm_year, 1 + date.tm_mon, date.tm_mday, date.tm_hour,
...
struct AdaptiveStreamer : ModuleDynI {
  ...
  AdaptiveStreamer(KHost *host,
        bool live,
        uint64_t segDurationInMs,
        const std::string &manifestDir,
        uint64_t multiPeriodFoldersInMs,
    ...
    if(!manifestDir.empty() && (flags & SegmentsNotOwned))
      throw error(format(
            "Inconsistent parameters: manifestDir (%s) should be empty when segments are not owned.", manifestDir));
    addInput();
  ...
  std::string
  getSegmentName(Quality const &quality, size_t index, int64_t startTime, const std::string &segmentNumSymbol) const {
  ...
  std::string getPrefix(Quality const &quality, size_t index) const {
    ...
    case AUDIO_PKT:
      return quality.prefix + format("a_%s", index);
    case VIDEO_PKT:
      return quality.prefix +
            format("v_%s_%sx%s", index, quality.getMeta()->resolution.width, quality.getMeta()->resolution.height);
    case SUBTITLE_PKT:
      return quality.prefix + format("s_%s", index);
    default:
      return "";
    }
  }

  uint64_t getCurSegNum() const { return (startTimeInMs + totalDurationInMs) / segDurationInMs; }
  ...
  int numInputs() { return getNumInputs() - 1; }
  ...
  bool isComplete(int repIdx) const {
    ...
    return (minIncompletSegDur == std::numeric_limits<uint64_t>::max()) ||
          (qualities[repIdx].curSegDurIn180k > minIncompletSegDur &&
                (qualities[repIdx].getMeta() && qualities[repIdx].getMeta()->EOS));
  }
  ...
  void sendLocalData(Data currData, int repIdx, uint64_t size, bool EOS) {
    ...
    if(out) {
      ...
      auto metaFn = make_shared<MetadataFile>(SEGMENT);
      metaFn->filename =
            getSegmentName(qualities[repIdx], repIdx, getCurSegNum() * segDurationInMs, std::to_string(getCurSegNum()));
      metaFn->mimeType = meta->mimeType;
      metaFn->codecName = meta->codecName;
      metaFn->lang = meta->lang;
      metaFn->durationIn180k = meta->durationIn180k;
      ...
      out->setMetadata(metaFn);
      out->set(
            PresentationTime{totalDurationInMs + timescaleToClock((int64_t)qualities[repIdx].curSegDurIn180k, 1000)});
      outputSegments->post(out);
    }
  }
  ...
  bool scheduleRepresentation(int repIdx) {
    ...
    if(segDurationInMs && curDurIn180k) {
      ...
      quality.avg_bitrate_in_bps =
            ((meta->filesize * 8 * IClock::Rate) / meta->durationIn180k + quality.avg_bitrate_in_bps * numSeg) /
            (numSeg + 1);
    }
...
class Dasher : public AdaptiveStreamer {
  ...
  std::string createManifest(DasherConfig m_cfg, bool dryRun) {
    ...
    mpd.availabilityStartTime =
          segDurationInMs /*time at which the first segment is available*/ + m_cfg.initialOffsetInMs;
    ...
    for(auto &period : periods) {
      ...
      period.duration = (periodIdx == (int)periods.size()) ? mpd.mediaPresentationDuration - period.startTime
                                                           : m_cfg.multiPeriodFoldersInMs;
      ...
      for(auto repIdx : getInputs()) {
        ...
        if(!m_cfg.tileInfo.empty()) {
          ...
          supplementalProperty = format("%s,%s,%s,%s,%s,%s,%s", ti.sourceId, ti.objectX, ti.objectY, ti.objectWidth,
                ti.objectHeight, ti.totalWidth, ti.totalHeight);
        }
        ...
        if(live && meta->latencyIn180k) {
          as.availabilityTimeOffset =
                std::max(0.0, (segDurationInMs - clockToTimescale(meta->latencyIn180k, DASH_TIMESCALE)) / 1000.0);
  ...
  void postSegment(Quality &quality, std::string segFilename, std::string nextSegFilename) {
    ...
    auto consideredDurationIn180k =
          metaFn->durationIn180k ? metaFn->durationIn180k : timescaleToClock(segDurationInMs, 1000);
