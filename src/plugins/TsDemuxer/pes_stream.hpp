...
Metadata createMetadata(int mpegStreamType) {
  ...
  case 0x01:
    return make(VIDEO_PKT, "mpeg2video");
  case 0x02:
    return make(VIDEO_PKT, "mpeg2video");
  case 0x03:
    return make(AUDIO_PKT, "mp1");
  case 0x04:
    return make(AUDIO_PKT, "mp2");
  case 0x0f:
    return make(AUDIO_PKT, "aac_adts");
  case 0x11:
    return make(AUDIO_PKT, "aac_latm");
  case 0x1b:
    return make(VIDEO_PKT, "h264_annexb");
  case 0x24:
    return make(VIDEO_PKT, "hevc_annexb");
  case 0x81:
    return make(AUDIO_PKT, "ac3");
  case 0x84:
    return make(AUDIO_PKT, "eac3");
  case 0x06: { /*private*/
...
struct PesStream : Stream {
  ...
  struct MyVector {
    MyVector(int reservedSize) { m_vector.resize(reservedSize); }
    ...
    uint8_t &operator[](int i) { return m_vector[i]; }
    const uint8_t *data() const { return m_vector.data(); }
    void clear() {
