...
class Scheduler : public IScheduler {
  public:
  Scheduler(std::shared_ptr<IClock> clock = g_SystemClock,
        std::shared_ptr<ITimer> timer = std::shared_ptr<ITimer>(new SystemTimer));
  Id scheduleAt(TaskFunc &&task, Fraction time) override;
  Id scheduleIn(TaskFunc &&task, Fraction time) override { return scheduleAt(std::move(task), clock->now() + time); }
