...
inline int64_t pgcd(int64_t a, int64_t b) { return b ? pgcd(b, a % b) : a; }
...
struct Fraction {
  Fraction(int64_t num = 1, int64_t den = 1)
      : num(num)
      , den(den) {
    if(!den)
  ...
  inline bool operator==(const Fraction &rhs) const { return num * rhs.den == rhs.num * den; }
