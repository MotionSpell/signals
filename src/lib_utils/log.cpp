...
void setGlobalLogger(LogSink &logger) { g_Log = &logger; }

static LogSink *getDefaultLogger() {
...
Level getGlobalLogLevel() { return g_Log->m_logLevel; }

void setGlobalLogLevel(Level level) { g_Log->setLevel(level); }
