...
static time_t pTimegm(struct tm *t) {
  ...
  static const int cumulatedDays[MONTHSPERYEAR] = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334};
...
Fraction parseDate(std::string s) {
  ...
  double second;
  int ret = sscanf(s.c_str(), "%04d-%02d-%02dT%02d:%02d:%lf%03d:%02dZ", &year, &month, &day, &hour, &minute, &second,
...
struct UtcClock : IUtcClock {
  Fraction getTime() override { return getUTC(); }
};
