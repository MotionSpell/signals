...
struct OffsetUtcClock : IUtcClock {
  Fraction getTime() { return g_UtcClock->getTime() - offset; }
  Fraction offset;
};
...
Resolution autoRotate(Resolution res, bool verticalize) {
  if(verticalize && res.height < res.width) {
    ...
    g_Log->log(Info,
          format("[autoRotate] Switched resolution from %sx%s to %sx%s", res.width, res.height, oRes.width, oRes.height)
                .c_str());
    return oRes;
  } else {
    return res;
...
Resolution autoFit(Resolution input, Resolution output) {
  if(input == Resolution())
    return output;
  if(output.width == -1) {
    assert((input.width * output.height % input.height) ==
          0); // TODO: add SAR at the DASH level to handle rounding errors
    Resolution oRes((input.width * output.height) / input.height, output.height);
    g_Log->log(Info,
          format("[autoFit] Switched resolution from -1x%s to %sx%s", input.height, oRes.width, oRes.height).c_str());
    return oRes;
  } else if(output.height == -1) {
    assert((input.height * output.width % input.width) ==
          0); // TODO: add SAR at the DASH level to handle rounding errors
    Resolution oRes(output.width, (input.height * output.width) / input.width);
    g_Log->log(Info,
          format("[autoFit] Switched resolution from %sx-1 to %sx%s", input.width, oRes.width, oRes.height).c_str());
    return oRes;
  } else {
    return output;
  }
...
IFilter *createEncoder(Pipeline *pipeline,
      Metadata metadata,
      bool ultraLowLatency,
      VideoCodecType videoCodecType,
      PictureFormat &dstFmt,
      int bitrate,
  ...
  if(codecType == VIDEO_PKT) {
    ...
    if((segmentDurationInMs * p.frameRate.num) % (1000 * GOPDurationDivisor * p.frameRate.den)) {
      g_Log->log(Warning,
            format("[%s] Couldn't align GOP size (%s/%s, divisor=%s) with segment duration (%sms). Segment duration "
                   "may vary.",
                  g_appName, p.GOPSize.num, p.GOPSize.den, GOPDurationDivisor, segmentDurationInMs)
                  .c_str());
    }
    if(GOPDurationDivisor > 1)
      g_Log->log(Info,
            format("[Encoder] Setting GOP duration to %sms (%s frames)", segmentDurationInMs / GOPDurationDivisor,
                  (double)p.GOPSize)
                  .c_str());
...
IFilter *createConverter(Pipeline *pipeline, Metadata metadata, const PictureFormat &dstFmt) {
  ...
  if(codecType == VIDEO_PKT) {
    ...
    auto format = PcmFormat(demuxFmt.sampleRate, demuxFmt.numChannels, demuxFmt.layout, demuxFmt.sampleFormat,
          (demuxFmt.numPlanes == 1) ? Interleaved : Planar);
...
struct Logger : LogSink {
  void send(Level level, const char *msg) override { g_Log->log(level, format("[%s] %s", g_appName, msg).c_str()); }
};
...
std::unique_ptr<Pipeline> buildPipeline(const Config &cfg) {
  ...
  auto dasherCfg = Modules::DasherConfig{DASH_SUBDIR, format("%s.mpd", g_appName), live,
        (uint64_t)cfg.segmentDurationInMs, cfg.segmentDurationInMs * cfg.timeshiftInSegNum};
  ...
  auto processElementaryStream = [&](int streamIndex) {
    ...
    for(int r = 0; r < numRes; ++r) {
      ...
      if(transcode) {
        ...
        auto encoder = createEncoder(pipeline.get(), metadata, cfg.ultraLowLatency, (VideoCodecType)cfg.v[r].type,
              encoderInputPicFmt, cfg.v[r].bitrate, cfg.segmentDurationInMs);
        if(!encoder)
          return;
