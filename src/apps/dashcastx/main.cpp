...
static void parseValue(Video &var, ArgQueue &args) {
  ...
  auto const parsed = sscanf(word.c_str(), "%dx%d:%d:%d", &var.res.width, &var.res.height, &var.bitrate, &var.type);
...
Config parseCommandLine(int argc, char const *argv[]) {
  ...
  opt.add("p", "publish", &cfg.publishUrl, "Publication URL (HTTP POST)");
  opt.add("s", "seg-dur", &cfg.segmentDurationInMs, "Set the segment duration (in ms) (default value: 2000).");
  opt.add("t", "dvr", &cfg.timeshiftInSegNum,
        "Set the timeshift buffer depth in segment number (default value: infinite(0)).");
  opt.add("v", "video", &cfg.v,
        "Set a video resolution and optionally bitrate (wxh[:b[:t]]) (enables resize and/or transcoding) and encoder "
        "type (supported 0 (software (default)), 1 (QuickSync), 2 (NVEnc).");
  ...
  opt.addFlag("u", "ultra-low-latency", &cfg.ultraLowLatency,
        "Lower the latency as much as possible (quality may be degraded).");
  ...
  opt.addFlag("m", "monitor", &cfg.debugMonitor,
        "Show the transcoded video in monitor window (slows the transcoding down to live speed)");
  ...
  if(cfg.help) {
    ...
    auto const examples = "\nExamples:\n"
          ...
          "  " +
          name +
          " --live --loop --seg-dur 10000 --dvr 10 --autorotate --video 320x180:50000 --video 640x360:300000 "
          "http://server.com/file.mp4\n"
          "  " +
          name +
          " --live -v 1280x720:1000000 webcam:video=/dev/video0:audio=/dev/audio1\n"
          "  " +
          name +
          " --live --working-dir workdir -v 640x360:300000 -v 1280x720:1000000 "
          "webcam:video=/dev/video0:audio=/dev/audio1\n"
          "  " +
          name + " -i -l -r -w tmp -t 10 -v 640x360:300000:0 udp://226.0.0.1:1234\n";
    printf("%s\n", examples.c_str());
