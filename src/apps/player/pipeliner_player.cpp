...
struct Restamper : Module {
  Restamper(KHost *, int count, int shift)
      : shift(shift) {
...
bool startsWith(std::string s, std::string prefix) { return s.substr(0, prefix.size()) == prefix; }

bool endsWith(std::string s, std::string suffix) { return s.substr(s.size() - suffix.size(), suffix.size()) == suffix; }
...
IFilter *createDemuxer(Pipeline &pipeline, std::string url) {
  if(startsWith(url, "mpegts://")) {
    ...
    } else if(sscanf(url.c_str(), "%d.%d.%d.%d:%d", &mcast.ipAddr[0], &mcast.ipAddr[1], &mcast.ipAddr[2],
                    &mcast.ipAddr[3], &mcast.port) == 5) {
...
void declarePipeline(Config cfg, Pipeline &pipeline, const char *url) {
  ...
  auto restamper = pipeline.addNamedModule<Restamper>(
        "restamper", demuxer->getNumOutputs(), timescaleToClock(maxMediaTimeDelayInMs - 100, 1000));
  ...
  if(rectify) {
    ...
    g_Log->log(Debug,
          format("Rectify at frequency %s/%s (%s)", rCfg.frameRate.num, rCfg.frameRate.den, (double)rCfg.frameRate)
                .c_str());
