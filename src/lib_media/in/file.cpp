...
class FileInput : public Module {
  public:
  FileInput(KHost *host, FileInputConfig const &config)
      : m_host(host) {
    ...
    if(size > m_blockSize)
      m_host->log(Info,
            format("File %s size is %s, will be sent by %s bytes chunks. Check the downstream modules are able to "
                   "aggregate data frames.",
                  config.filename, size, m_blockSize)
                  .c_str());
