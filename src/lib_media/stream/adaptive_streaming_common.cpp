...
AdaptiveStreamingCommon::AdaptiveStreamingCommon(KHost *host,
      Type type,
...
std::string AdaptiveStreamingCommon::getPrefix(Quality const *const quality, size_t index) const {
  switch(quality->getMeta()->type) {
  case AUDIO_PKT:
    return format("%s%s", quality->prefix, getCommonPrefixAudio(index));
  case VIDEO_PKT:
    return format("%s%s", quality->prefix, getCommonPrefixVideo(index, quality->getMeta()->resolution));
  case SUBTITLE_PKT:
    return format("%s%s", quality->prefix, getCommonPrefixSubtitle(index));
  default:
    return "";
  }
}

std::string AdaptiveStreamingCommon::getSegmentName(Quality const *const quality,
      size_t index,
      const std::string &segmentNumSymbol) const {
  switch(quality->getMeta()->type) {
  case AUDIO_PKT:
  case VIDEO_PKT:
  case SUBTITLE_PKT:
...
uint64_t AdaptiveStreamingCommon::getCurSegNum() const { return (startTimeInMs + totalDurationInMs) / segDurationInMs; }
...
void AdaptiveStreamingCommon::threadProc() {
  ...
  auto isComplete = [&]() -> bool {
    ...
    for(size_t idx = 0; idx < curSegDurIn180k.size(); ++idx) {
      ...
      if((segDur < minIncompletSegDur) &&
            ((segDur < timescaleToClock(segDurationInMs, 1000)) ||
                  (!qualities[idx]->getMeta() || !qualities[idx]->getMeta()->EOS))) {
  ...
  auto ensureStartTime = [&]() {
    if(startTimeInMs == (uint64_t)-2)
      startTimeInMs = clockToTimescale(data->get<PresentationTime>().time, 1000);
  };
  auto ensureCurDur = [&]() {
  ...
  for(;;) {
    for(i = 0; i < numInputs; ++i) {
      ...
      if(data) {
        ...
        if(segDurationInMs && curDurIn180k) {
          ...
          qualities[i]->avg_bitrate_in_bps = ((meta->filesize * 8 * IClock::Rate) / meta->durationIn180k +
                                                   qualities[i]->avg_bitrate_in_bps * numSeg) /
                (numSeg + 1);
        }
    ...
    if(segmentReady()) {
      ...
      m_host->log(Info,
            format("Processes segment (total processed: %ss, UTC: %sms (deltaAST=%s, deltaInput=%s).",
                  (double)totalDurationInMs / 1000, utcInMs, utcInMs - startTimeInMs,
                  (int64_t)(utcInMs - curMediaTimeInMs))
                  .c_str());
...
void AdaptiveStreamingCommon::flush() { endOfStream(); }
