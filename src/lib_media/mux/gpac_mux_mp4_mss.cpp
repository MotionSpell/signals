...
class GPACMuxMP4MSS : public GPACMuxMP4 {
  ...
  std::string writeISMLManifest(std::string codec4CC,
...
GPACMuxMP4MSS::GPACMuxMP4MSS(KHost *host, Mp4MuxConfigMss &cfg)
    : GPACMuxMP4(host,
            Mp4MuxConfig{
                  ...
                  SmoothStreaming | Browsers | NoEditLists | (!cfg.audioName.empty() ? SegConstantDur : None) |
                        ((!cfg.audioLang.empty() || cfg.audioName.empty()) ? ExactInputDur : None),
                  cfg.utcStartTime,
            })
...
void GPACMuxMP4MSS::declareStreamAudio(const MetadataPktAudio *metadata) {
  ...
  ISMLManifest = writeISMLManifest(codec4CC, string2hex(extradata.ptr, extradata.len), metadata->bitrate, 0, 0,
        metadata->sampleRate, metadata->numChannels, bitsPerSample);
  for(int k = 0; k < 4; ++k)
    ISMLManifest[k] = 0;
}
...
void GPACMuxMP4MSS::declareStreamVideo(const MetadataPktVideo *metadata) {
  ...
  ISMLManifest = writeISMLManifest(
        codec4CC, string2hex(extradata.ptr, extradata.len), metadata->bitrate, res.width, res.height, 0, 0, 0);
  for(int k = 0; k < 4; ++k)
    ISMLManifest[k] = 0;
}
...
std::string GPACMuxMP4MSS::writeISMLManifest(std::string codec4CC,
      std::string codecPrivate,
      int64_t bitrate,
      int width,
      int height,
  ...
  // TODO: multiple tracks for (i=0; i<nbTracks; ++i)
  {
    ...
    if(type == "audio") {
      ...
      if(!audioName.empty())
        ss << "        <param name=\"trackName\" value=\"" << audioName << "\" valuetype=\"data\" />\n";
      if(!lang.empty())
        ss << "        <param name=\"systemLanguage\" value=\"" << lang << "\" valuetype=\"data\" />\n";
      ...
      if(!audioName.empty())
        ss << "        <param name=\"trackName\" value=\"" << audioName << "\" valuetype=\"data\" />\n";
      if(!audioLang.empty())
        ss << "        <param name=\"systemLanguage\" value=\"" << audioLang << "\" valuetype=\"data\" />\n";
    } else
