...
static GF_Err import_extradata_avc(SpanC extradata, GF_AVCConfig *dstcfg) {
  ...
  auto bs2 = std::shared_ptr<GF_BitStream>(
        gf_bs_new((const char *)extradata.ptr, extradata.len, GF_BITSTREAM_READ), &gf_bs_del);
  auto bs = bs2.get();
  if(!bs) {
    return GF_BAD_PARAM;
  }
...
GPACMuxMP4::GPACMuxMP4(KHost *host, Mp4MuxConfig const &cfg)
  ...
  if((compatFlags & FlushFragMemory) && (!cfg.baseName.empty() || segmentPolicy != FragmentedSegment))
    throw error(
          "Inconsistent parameters: FlushFragMemory requires an empty segment name and FragmentedSegment policy.");
...
GPACMuxMP4::~GPACMuxMP4() { gf_isom_delete(isoCur); }
...
void GPACMuxMP4::closeSegment(bool isLastSeg) {
  ...
  if(segmentPolicy == FragmentedSegment) {
    GF_Err e =
          gf_isom_close_segment(isoCur, 0, 0, 0, 0, 0, GF_FALSE, GF_FALSE, (Bool)isLastSeg, (Bool)(!initName.empty()),
  ...
  m_host->log(Debug,
        format("Segment %s completed (size %s) (startsWithSAP=%s)", segmentName.empty() ? "[in memory]" : segmentName,
              lastSegmentSize, segmentStartsWithRAP)
              .c_str());
...
void GPACMuxMP4::closeFragment() {
  if(!curFragmentDurInTs) {
    m_host->log(
          (compatFlags & Browsers) ? Error : Warning, "Writing an empty fragment. Some players may stop playing here.");
  }
  ...
  if(compatFlags & SmoothStreaming) {
    ...
    {
      auto const isSuspicious = deltaRealTimeInMs < 0 || deltaRealTimeInMs > curFragmentStartInTs ||
            curFragmentDurInTs != fractionToTimescale(segmentDuration, timeScale);
      m_host->log(isSuspicious ? Warning : Debug,
            format("Closing MSS fragment with absolute time %s %s UTC and duration %s (timescale %s, time=%s, "
                   "deltaRT=%s)",
                  getDay(), getTimeFromUTC(), curFragmentDurInTs, timeScale, absTimeInTs, deltaRealTimeInMs)
                  .c_str());
    }
...
void GPACMuxMP4::setupFragments() {
  if(fragmentPolicy > NoFragment) {
    SAFE(gf_isom_setup_track_fragment(
          isoCur, trackId, 1, compatFlags & SmoothStreaming ? 0 : (u32)defaultSampleIncInTs, 0, 0, 0, 0, 0));
...
static auto deleteEsd = [](GF_ESD *p) { gf_odf_desc_del((GF_Descriptor *)p); };
...
void GPACMuxMP4::declareStreamAudio(const MetadataPktAudio *metadata) {
  ...
  if(metadata->codec == "aac_raw") {
    ...
    SAFE(gf_m4a_write_config(
          &acfg, &esd->decoderConfig->decoderSpecificInfo->data, &esd->decoderConfig->decoderSpecificInfo->dataLength));
    ...
    if(!bs)
      throw error(format(
            "(E)AC-3: impossible to create extradata bitstream (\"%s\", size=%s)", metadata->codec, extradata.len));

    GF_AC3Header hdr{};
    if(is_EAC3 || !gf_ac3_parser_bs(bs, &hdr, GF_TRUE)) {
      if(!gf_eac3_parser_bs(bs, &hdr, GF_TRUE)) {
        m_host->log(Error,
              format("Parsing: audio is neither AC3 or E-AC3 audio (\"%s\", size=%s)", metadata->codec, extradata.len)
                    .c_str());
      }
    }
  ...
  SAFE(gf_isom_set_audio_info(
        isoCur, trackNum, di, sampleRate, metadata->numChannels, bitsPerSample, GF_IMPORT_AUDIO_SAMPLE_ENTRY_NOT_SET));
...
void GPACMuxMP4::declareStreamVideo(const MetadataPktVideo *metadata) {
  ...
  if(metadata->codec == "h264_annexb" || metadata->codec == "h264_avcc") {
    ...
    isAnnexB = false;
    m_host->log(Warning,
          format("Using generic packaging for codec '%s%s%s%s'", (char)((MP4_4CC >> 24) & 0xff),
                (char)((MP4_4CC >> 16) & 0xff), (char)((MP4_4CC >> 8) & 0xff), (char)(MP4_4CC & 0xff))
                .c_str());
    ...
    if(!sdesc.vendor_code)
      sdesc.vendor_code = GF_VENDOR_GPAC;
  ...
  if(e) {
    if(e == GF_NON_COMPLIANT_BITSTREAM) {
      ...
      esd.decoderConfig->objectTypeIndication = metadata->codec == "h264_annexb" || metadata->codec == "h264_avcc"
            ? GPAC_OTI_VIDEO_AVC
            : GPAC_OTI_VIDEO_HEVC;
...
void GPACMuxMP4::handleInitialTimeOffset() {
  // FIXME: we use DTS here: that's convenient (because the DTS is monotonic) and *most of the time* right when we
  // control the encoding
  if(initDTSIn180k) { /*first timestamp is not zero*/
    m_host->log(Info,
          format("Initial offset: %ss (4CC=%s, \"%s\", timescale=%s/%s)", initDTSIn180k / (double)IClock::Rate,
                codec4CC, segmentName, timeScale, gf_isom_get_timescale(isoCur))
                .c_str());
    if(compatFlags & NoEditLists) {
      ...
      if(edtsInMovieTs > 0) {
        gf_isom_append_edit_segment(
              isoCur, gf_isom_get_track_by_id(isoCur, trackId), edtsInMovieTs, 0, GF_ISOM_EDIT_EMPTY);
        gf_isom_append_edit_segment(
              isoCur, gf_isom_get_track_by_id(isoCur, trackId), edtsInMovieTs, 0, GF_ISOM_EDIT_NORMAL);
        curSegmentDeltaInTs = edtsInMediaTs;
      } else {
        gf_isom_append_edit_segment(
              isoCur, gf_isom_get_track_by_id(isoCur, trackId), 0, -edtsInMediaTs, GF_ISOM_EDIT_NORMAL);
      }
    }
  }
}
...
void GPACMuxMP4::sendSegmentToOutput(bool EOS) {
  ...
  case GF_ISOM_MEDIA_VISUAL:
    streamType = VIDEO_PKT;
    mimeType = "video/mp4";
    break;
  case GF_ISOM_MEDIA_AUDIO:
    streamType = AUDIO_PKT;
    mimeType = "audio/mp4";
    break;
  case GF_ISOM_MEDIA_TEXT:
    streamType = SUBTITLE_PKT;
    mimeType = "application/mp4";
    break;
  default:
    throw error(format("Unknown media type for segment: %s", (int)mediaType));
  }
  Bool isInband = AVC_INBAND_CONFIG ? GF_TRUE : GF_FALSE;
  char codecName[40];
  GF_Err e = gf_media_get_rfc_6381_codec_name(
        isoCur, gf_isom_get_track_by_id(isoCur, trackId), codecName, isInband, GF_FALSE);
  if(e)
  ...
  auto const containerLatency = fragmentPolicy == OneFragmentPerFrame
        ? timescaleToClock(defaultSampleIncInTs, timeScale)
        : std::min<uint64_t>(consideredDurationIn180k, fractionToClock(segmentDuration));
...
void GPACMuxMP4::startChunk(gpacpp::IsoSample *const sample) {
  ...
  if(segmentPolicy > SingleSegment) {
    ...
    if(oneSegDurInTs * (m_DTS / oneSegDurInTs) == 0) { /*initial delay*/
      curSegmentDeltaInTs = curSegmentDurInTs + curSegmentDeltaInTs -
            oneSegDurInTs * ((curSegmentDurInTs + curSegmentDeltaInTs) / oneSegDurInTs);
    } else {
...
void GPACMuxMP4::closeChunk(bool nextSampleIsRAP) {
  ...
  if((!(compatFlags & Browsers) || curFragmentDurInTs > 0 ||
           fragmentPolicy == OneFragmentPerFrame) && /*avoid 0-sized mdat interpreted as EOS in browsers*/
...
static bool isRap(Data data) { return data->get<CueFlags>().keyframe; }
...
void GPACMuxMP4::fillSample(Data data, gpacpp::IsoSample *sample, bool isRap) {
  ...
  if(data->get<PresentationTime>().time != INT64_MAX) {
    ...
    m_host->log(Error,
          format("Missing PTS (input DTS=%s, ts=%s/%s): output MP4 may be incorrect.", data->get<DecodingTime>().time,
                srcTimeScale.num, srcTimeScale.den)
                .c_str());
  }
  ...
  if(compatFlags & ExactInputDur) {
    if(lastData) {
      if(dataDurationInTs <= 0) {
        m_host->log(Warning,
              format("Computed duration is inferior or equal to zero (%s). Inferring to %s", dataDurationInTs,
                    defaultSampleIncInTs)
                    .c_str());
    ...
    if(m_DTS > 0) {
      ...
      if(lastDataDurationInTs != defaultSampleIncInTs) {
        ...
        m_host->log(
              Debug, format("VFR: adding sample with duration %ss", lastDataDurationInTs / (double)timeScale).c_str());
      }
    }
