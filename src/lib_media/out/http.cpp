...
namespace {
bool startsWith(std::string s, std::string prefix) { return !s.compare(0, prefix.size(), prefix); }

class HTTP : public ModuleS {
  public:
  HTTP(KHost *host, HttpOutputConfig const &cfg)
      : m_host(host)
      , m_suffixData(cfg.endOfSessionSuffix) {
    ...
    m_sender =
          createHttpSender({cfg.url, cfg.userAgent, cfg.flags.request, cfg.headers, cfg.maxConnectFailCount}, m_host);
  }
