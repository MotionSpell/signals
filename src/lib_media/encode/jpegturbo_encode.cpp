...
class JPEGTurboEncode : public ModuleS {
  ...
  JPEGTurboEncode(KHost *host_, int quality = JPEG_DEFAULT_QUALITY)
      : m_host(host_)
      , jtHandle(tjInitCompress())
      , quality(quality) {
    input->setMetadata(make_shared<MetadataRawVideo>());
    output = addOutput();
  }

  ~JPEGTurboEncode() { tjDestroy(jtHandle); }
  ...
  void processOne(Data data_) override {
    ...
    case PixelFormat::NV12: {
      ...
      if(tjCompressFromYUVPlanes(jtHandle, srcSlice, videoData->getFormat().res.width, srcStride,
               videoData->getFormat().res.height, TJSAMP_420, &buf, &jpegSize, quality,
    ...
    case PixelFormat::RGBA32: {
      if(tjCompress2(jtHandle, (unsigned char *)jpegBuf, dim.width, 0 /*pitch*/, dim.height, TJPF_RGB, &buf, &jpegSize,
               TJSAMP_420, quality, TJFLAG_NOREALLOC | TJFLAG_FASTDCT) < 0) {
