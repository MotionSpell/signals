...
AVRational toAVRational(Fraction f) { return {(int)f.num, (int)f.den}; }
  ...
  LibavEncode(KHost *host, EncoderConfig *pparams)
    ...
    default:
      const char *typeStr = (type == EncoderConfig::Video) ? "Video"
            : (type == EncoderConfig::Audio)               ? "Audio"
                                                           : "Unknown";
    ...
    if(f->pts == std::numeric_limits<int64_t>::min()) {
      ...
      if(prevGOP != currGOP) {
        if(currGOP != prevGOP + 1) {
          m_host->log(Warning,
                format("Invalid content: switching from GOP %s to GOP %s - inserting RAP.", prevGOP, currGOP).c_str());
        }
        f->key_frame = 1;
        f->pict_type = AV_PICTURE_TYPE_I;
      } else {
  ...
  void openEncoder(Data data) {
    ...
    default:
      const char *typeStr = (params.type == EncoderConfig::Video) ? "Video"
            : (params.type == EncoderConfig::Audio)               ? "Audio"
                                                                  : "Unknown";
      throw error(format("Invalid codec type: %s", typeStr));
    }
