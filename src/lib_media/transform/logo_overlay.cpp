...
static uint8_t blend(uint8_t a, uint8_t b, int alpha) { return (alpha * b + (256 - alpha) * a) >> 8; }
...
static void compose(DataPicture *pic, int x, int y, const DataPicture *overlay, const DataPicture *mask) {
...
class LogoOverlay : public Module {
  ...
  void processOverlay(std::shared_ptr<const DataPicture> overlay) { m_overlay = overlay; }
  ...
  void createOverlay(PixelFormat pixelFormat) {
    struct OutStub : ModuleS {
      ...
      void processOne(Data data) override { fn(data); }
    };
      ...
      res.height = m_cfg.dim.height;
    {
      ...
      OutStub stub;
      stub.fn = [this](Data data) { this->m_convertedOverlay = safe_cast<const DataPicture>(data); };
    ...
    if(m_overlay->getFormat().hasTransparency()) {
      OutStub stub;
      stub.fn = [this](Data data) { this->m_overlayMask = safe_cast<const DataPicture>(data); };
