...
struct AudioGapFiller : public ModuleS {
  ...
  void processOne(Data data) override {
    ...
    if(std::abs(diff) >= srcNumSamples) {
      if(toleranceInFrames == -1 ||
            (toleranceInFrames > 0 && std::abs(diff) <= srcNumSamples * (1 + (int64_t)toleranceInFrames))) {
        if(diff > 0) {
          m_host->log(abs(diff) > srcNumSamples ? Warning : Debug,
                format("Fixing gap of %s samples (input=%s, accumulation=%s)", diff, timeInSR, accumulatedTimeInSR)
                      .c_str());
          auto dataInThePast = data->clone();
          dataInThePast->set(
                PresentationTime{data->get<PresentationTime>().time - timescaleToClock(srcNumSamples, sampleRate)});
        ...
        m_host->log(Warning,
              format("Discontinuity detected. Reset at time %s (previous: %s).", data->get<PresentationTime>().time,
                    timescaleToClock(accumulatedTimeInSR, sampleRate))
                    .c_str());
        accumulatedTimeInSR = timeInSR;
      }
    }
