...
static size_t ALIGN_PAD(size_t n, size_t align) { return ((n + align - 1) / align) * align; }
...
class VideoConvert : public ModuleS {
  ...
  ~VideoConvert() { sws_freeContext(m_SwContext); }
  ...
  void reconfigure(const PictureFormat &srcFormat) {
    ...
    m_SwContext = sws_getContext(srcFormat.res.width, srcFormat.res.height, pixelFormat2libavPixFmt(srcFormat.format),
          dstFormat.res.width, dstFormat.res.height, pixelFormat2libavPixFmt(dstFormat.format), SWS_BILINEAR, nullptr,
          nullptr, nullptr);
    if(!m_SwContext)
      throw error("Impossible to set up video converter.");
    m_host->log(Info,
          format("Converter configured to: %sx%s:%s -> %sx%s:%s", srcFormat.res.width, srcFormat.res.height,
                (int)srcFormat.format, dstFormat.res.width, dstFormat.res.height, (int)dstFormat.format)
                .c_str());
