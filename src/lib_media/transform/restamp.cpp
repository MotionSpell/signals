...
namespace Modules { namespace Transform {
...
int64_t Restamp::restamp(int64_t time) {
  ...
  if(time + offset < 0) {
    if(time / IClock::Rate < 2) {
      m_host->log(Error,
            format("reset offset [%s -> %ss (time=%s, offset=%s)]", (double)time / IClock::Rate,
                  (double)(std::max<int64_t>(0, time + offset)) / IClock::Rate, time, offset)
                  .c_str());
      offset = 0;
    }
  }
...
void Restamp::processOne(Data data) {
  ...
  m_host->log(((time != 0) && (time + offset < 0)) ? Info : Debug,
        format("%s -> %ss (time=%s, offset=%s)", (double)time / IClock::Rate, (double)(restampedTime) / IClock::Rate,
              time, offset)
              .c_str());
