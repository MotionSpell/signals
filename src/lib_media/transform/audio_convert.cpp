...
namespace {
struct Resampler {
  Resampler() { m_SwrContext = swr_alloc(); }

  ~Resampler() { swr_free(&m_SwrContext); }
  ...
  int64_t getDelay(int64_t rate) { return swr_get_delay(m_SwrContext, rate); }
...
static std::string PcmFormatToString(PcmFormat fmt) {
  ...
  return format(
        "[%s channels, %s kHz, %s]", fmt.numChannels, fmt.sampleRate / 1000.0, sampleFormatToString(fmt.sampleFormat));
}
...
struct AudioConvert : ModuleS {
  ...
  void processOne(Data data) override {
    ...
    if(inputMediaTime != -1) {
      ...
      if(actualInputTime && clockToTimescale(std::abs(actualInputTime - expectedInputTime), 1000) > 25) {
        m_host->log(Warning,
              format("input gap: %sms", (actualInputTime - expectedInputTime) * 1000.0 / IClock::Rate).c_str());
        resyncNeeded = true;
      }
    }
  ...
  bool doConvert(int targetNumSamples, const void *pSrc, int srcNumSamples) {
    ...
    if(outNumSamples > targetNumSamples)
      throw error(format("Unexpected case: output %s samples when %s was requested (frame size = %s)", outNumSamples,
            targetNumSamples, m_dstLen));
  ...
  void configure(const PcmFormat &srcFormat) {
    ...
    int ret = swr_alloc_set_opts2(&m_resampler->m_SwrContext, &avDstChannelLayout, avDstFmt, avDstSampleRate,
          &avSrcChannelLayout, avSrcFmt, avSrcSampleRate, 0, nullptr);
    ...
    m_host->log(Info,
          format("Converter configured to: %s -> %s", PcmFormatToString(srcFormat), PcmFormatToString(m_dstFormat))
                .c_str());
  }
...
IModule *createObject(KHost *host, void *va) {
  ...
  enforce(samples == -1 || (samples >= 0 && samples < 1024 * 1024),
        format("AudioConvert: sample count (%s) must be valid", samples).c_str());
  if(src.sampleRate == 0)
