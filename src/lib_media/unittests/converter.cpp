...
struct Recorder : ModuleS {
  void processOne(Data frame_) override { out = frame_; }
  Data out;
};
}
...
void framingTest(int inSamplesPerFrame, int outSamplesPerFrame) {
  ...
  const int expectedTotalSize =
        divUp(inSamplesPerFrame * numIter, outSamplesPerFrame) * outSamplesPerFrame * format.getBytesPerSample();
...
unittest("audio converter: timestamp passthrough") {
  ...
  auto onFrame = [&](Data dataRec) { lastMediaTime = dataRec->get<PresentationTime>().time; };
...
unittest("audio gap filler") {
  ...
  while(auto dataRec = recorder->pop()) {
    ASSERT(std::abs(dataRec->get<PresentationTime>().time -
                 timescaleToClock(out[idx] * numSamples, format.sampleRate)) < 6);
    idx++;
  }
  ASSERT(idx == out.size());
}
