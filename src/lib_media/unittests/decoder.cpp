...
unittest("decoder: timestamp propagation") {
  struct FrameCounter : ModuleS {
    void processOne(Data data) override { mediaTimes.push_back(data->get<PresentationTime>().time); }
...
unittest("decoder: video simple") {
  ...
  auto onPic = [&](Data data) {
    ...
    char info[256];
    sprintf(
          info, "'%dx%d %d %.2X %.2X'", format.res.width, format.res.height, (int)format.format, firstPixel, lastPixel);
