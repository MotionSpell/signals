...
unittest("mux GPAC mp4 failure tests") {
  ...
  {
    auto cfg =
          Mp4MuxConfig{"", segmentDurationInMs, IndependentSegment, NoFragment, SegNumStartsAtZero | FlushFragMemory};
    ASSERT_THROWN(loadModule("GPACMuxMP4", &NullHost, &cfg););
  }
  {
    auto cfg = Mp4MuxConfig{
          "out/output_video_gpac_20", segmentDurationInMs, FragmentedSegment, NoFragment, SegNumStartsAtZero};
    ASSERT_THROWN(loadModule("GPACMuxMP4", &NullHost, &cfg););
  }
}

static IOutput *getFirstAudioPin(IModule *demux) {
  for(int i = 0; i < demux->getNumOutputs(); ++i) {
...
static std::vector<Meta> runMux(std::shared_ptr<IModule> m) {
  DemuxConfig cfg;
  cfg.url = "data/beepbop.mp4";
  auto demux = loadModule("LibavDemux", &NullHost, &cfg);
...
unittest("mux GPAC mp4 combination coverage 1") {
  ...
  auto cfg2 = Mp4MuxConfig{"output_video_gpac_12", segmentDurationInMs, IndependentSegment, OneFragmentPerSegment,
        SegNumStartsAtZero | Browsers};
  auto cfg3 = Mp4MuxConfig{"output_video_gpac_13", segmentDurationInMs, IndependentSegment, OneFragmentPerRAP,
        SegNumStartsAtZero | Browsers};
  auto cfg4 = Mp4MuxConfig{"output_video_gpac_14", segmentDurationInMs, IndependentSegment, OneFragmentPerFrame,
        SegNumStartsAtZero | Browsers};
...
unittest("mux GPAC mp4 combination coverage 2") {
  ...
  auto cfg3 = Mp4MuxConfig{
        "", segmentDurationInMs, FragmentedSegment, OneFragmentPerSegment, SegNumStartsAtZero | FlushFragMemory};

  ASSERT_EQUALS(ref,
        runMux(loadModule("GPACMuxMP4", &NullHost, &cfg1)) + runMux(loadModule("GPACMuxMP4", &NullHost, &cfg2)) +
