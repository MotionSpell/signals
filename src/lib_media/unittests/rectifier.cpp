...
class ClockMock : public IClock, public IScheduler {
  ...
  Fraction now() const override { return m_time; }
  ...
  struct Task {
    Fraction time;
    TaskFunc func;
    bool operator<(Task const &other) const { return time < other.time; }
  };
...
struct Fixture {
  ...
  void setTime(int64_t time) { clock->setTime(Fraction(time, IClock::Rate)); }

  void addStream(int i, unique_ptr<ModuleS> &&generator) {
    generators.push_back(std::move(generator));

    ConnectModules(generators[i].get(), 0, rectifier.get(), i);
    ConnectOutput(rectifier->getOutput(i), [i, this](Data data) { this->onOutputSample(i, data); });
  }
  ...
  void onOutputSample(int i, Data data) {
    ...
    if(dataPcm) {
      ...
      for(int i = 0; i < dataPcm->getSampleCount() * dataPcm->format.getBytesPerSample() / (int)sizeof(uint32_t); ++i) {
        ...
        if(!initialPadding ||
              val != 0) { // initial unmatched samples contain zeroes instead of the counter: exclude them
...
vector<Event> runRectifier(Fraction fps, vector<unique_ptr<ModuleS>> &gen, vector<Event> events) {
...
vector<Event> generateEvents(Fraction fps,
      int index = 0,
      function<TimePair(uint64_t, Fraction)> generateValue = generateTimePairDefault) {
...
vector<Event> generateInterleavedEvents(Fraction frameDurAudio,
      int clockOffsetAudio,
      int mediaOffsetAudio,
      Fraction frameDurVideo,
      int clockOffsetVideo,
      int mediaOffsetVideo) {
...
unittest("rectifier: FPS factor (single port) 9 fps, x1") { testFPSFactor(9, 1); }

unittest("rectifier: FPS factor (single port) 9 fps, x2") { testFPSFactor(9, 2); }

unittest("rectifier: FPS factor (single port) 25 fps, x1/2") { testFPSFactor(25, Fraction(1, 2)); }

unittest("rectifier: FPS factor (single port) 29.97 fps, x1") { testFPSFactor(Fraction(30000, 1001), 1); }

unittest("rectifier: FPS factor (single port) 29.97 fps, x2") { testFPSFactor(Fraction(30000, 1001), 2); }
...
unittest("rectifier: deal with backward discontinuity (single port)") {
  ...
  auto inTimes2 =
        generateEvents(fps, 0, bind(generateTimePair, placeholders::_1, placeholders::_2, inTimes1.size(), 0));
  auto outTimes1 = generateEvents(fps);
  auto outTimes2 = generateEvents(
        fps, 0, bind(generateTimePair, placeholders::_1, placeholders::_2, inTimes1.size(), inTimes1.size()));
...
unittest("rectifier: multiple media types simple") {
  ...
  auto const expectedTimes = vector<Event>({
        Event{0, 1000 + 7200 * 0, 7200 * 0},
        Event{1, 1000 + 7200 * 0, 7200 * 0},
        Event{0, 1000 + 7200 * 1, 7200 * 1},
        Event{1, 1000 + 7200 * 1, 7200 * 1},
        Event{0, 1000 + 7200 * 2, 7200 * 2},
        Event{1, 1000 + 7200 * 2, 7200 * 2},
        Event{0, 1000 + 7200 * 3, 7200 * 3},
        Event{1, 1000 + 7200 * 3, 7200 * 3},
  });
...
unittest("rectifier: subtitles (sparse)") {
  ...
  auto const expectedTimes = vector<Event>({
        Event{0, offset + delta * 0, delta * 0},
        Event{1, offset + delta * 0, delta * 0} /*wrong subtitle*/,
        Event{1, offset + delta * 0, delta * 0} /*heartbeat*/,
        Event{0, offset + delta * 1, delta * 1},
        Event{1, offset + delta * 1, delta * 1} /*heartbeat*/,
        Event{0, offset + delta * 2, delta * 2},
        Event{1, offset + delta * 2, delta * 2} /*wrong subtitle*/,
        Event{1, offset + delta * 2, delta * 2} /*heartbeat*/,
        Event{0, offset + delta * 3, delta * 3},
        Event{1, offset + delta * 3, delta * 3} /*heartbeat*/,
  });
...
unittest("rectifier: audio 44100 interleave") {
  ...
  auto expected = vector<Event>({
        ...
        Event{1, 93600, 93600},
        Event{0, 93600, 93600},
        Event{1, 100800, 100800},
        Event{0, 100800, 100800},
  });
  ASSERT_EQUALS(expected, actualTimes);
}
