...
class FakeOutput : public Module {
  public:
  FakeOutput() { output = addOutput(); }
  void process() {
    PcmFormat fmt;
    auto data = output->allocData<DataPcm>(0, fmt);
    output->post(data);
  }
  void setMetadata(std::shared_ptr<const IMetadata> metadata) { output->setMetadata(metadata); }
...
class FakeInput : public Module {
  public:
  FakeInput() { input = addInput(); }
  void process() {
    auto data = inputs[0]->pop();
    inputs[0]->updateMetadata(data);
  }

  void setMetadata(std::shared_ptr<const IMetadata> metadata) { input->setMetadata(metadata); }
