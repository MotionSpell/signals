...
HardwareContextCuda::~HardwareContextCuda() { av_buffer_unref((AVBufferRef **)&device); }
...
static void initMetadatPkt(MetadataPkt *meta, AVCodecContext *codecCtx) {
  ...
  if(codecCtx->framerate.num == 0 && codecCtx->framerate.den == 0) {
    ...
    if(!codecCtx->framerate.num || !codecCtx->framerate.den)
      throw std::runtime_error(
            format("Unsupported video frame rate %s/%s.", codecCtx->framerate.den, codecCtx->framerate.num));
    meta->framerate = Fraction(codecCtx->framerate.num, codecCtx->framerate.den);
  }
  return meta;
}

static bool isPlanar(const AVCodecContext *codecCtx) {
  ...
  default:
    throw std::runtime_error(
          format("Unknown libav audio format [%s] (2)", av_get_sample_fmt_name(codecCtx->sample_fmt)));
  }
}

static AudioSampleFormat getFormat(const AVCodecContext *codecCtx) {
  switch(codecCtx->sample_fmt) {
  case AV_SAMPLE_FMT_S16:
    return Modules::S16;
  case AV_SAMPLE_FMT_S16P:
  ...
  default:
    throw std::runtime_error(
          format("Unknown libav audio format [%s] (3)", av_get_sample_fmt_name(codecCtx->sample_fmt)));
  }
}

static AudioLayout getLayout(const AVCodecContext *codecCtx) {
  ...
  default:
    ...
    case 6:
      return FivePointOne;
    default:
      throw std::runtime_error("Unknown libav audio layout");
    }
  }
#else
...
// conversions
void libavAudioCtxConvertLibav(const Modules::PcmFormat *cfg,
      int &sampleRate,
  ...
  case Modules::S16:
    format = av_get_alt_sample_fmt(AV_SAMPLE_FMT_S16, cfg->numPlanes > 1);
    break;
  case Modules::F32:
    format = av_get_alt_sample_fmt(AV_SAMPLE_FMT_FLT, cfg->numPlanes > 1);
    break;
  default:
    throw std::runtime_error("Unknown libav audio format (1)");
  }
}
...
void libavFrame2pcmConvert(const AVFrame *frame, PcmFormat *cfg) {
  ...
  case 1:
    cfg->layout = Modules::Mono;
    break;
  case 2:
    cfg->layout = Modules::Stereo;
    break;
  case 6:
    cfg->layout = Modules::FivePointOne;
    break;
  default:
    throw std::runtime_error("Unknown libav audio layout");
  }
...
PixelFormat libavPixFmt2PixelFormat(AVPixelFormat avPixfmt) {
  ...
  default:
    throw std::runtime_error(
          "Unsupported pixel format '" + std::string(av_get_pix_fmt_name(avPixfmt)) + "'. Please contact your vendor.");
  }
}
