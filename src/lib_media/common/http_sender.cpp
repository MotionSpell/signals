...
namespace {
size_t writeVoid(void *, size_t size, size_t nmemb, void *) { return size * nmemb; }

struct CurlScope {
  CurlScope() { curl_global_init(CURL_GLOBAL_ALL); }
  ~CurlScope() { curl_global_cleanup(); }
};
...
struct CurlHttpSender : HttpSender {
  CurlHttpSender(HttpSenderConfig const &cfg, Modules::KHost *log)
      : m_cfg(cfg) {
    m_log = log;
  }
  ...
  void send(span<const uint8_t> data) override {
    ...
    if(!data.len) {
      ...
      std::unique_lock<std::mutex> lock(m_mutex);
      auto pred = [this]() { return allDataSent || destroying || connectFailCountExceeded; };
      while(!pred())
        m_allDataSent.wait(lock, pred);
    }
  }

  void appendPrefix(span<const uint8_t> prefix) override { append(m_prefixData, prefix); }
  ...
  size_t fillBuffer(span<uint8_t> buffer) {
    ...
    // wait for new data
    auto pred = [this]() { return !m_fifo.empty() || endOfDataFlag || destroying; };
    while(!pred())
