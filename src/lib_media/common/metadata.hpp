...
struct MetadataRawSubtitle : IMetadata {
  MetadataRawSubtitle()
      : IMetadata(SUBTITLE_RAW) {}
};

struct MetadataPkt : public IMetadata {
  MetadataPkt(StreamType type)
      : IMetadata(type) {}
  ...
  SpanC getExtradata() const { return SpanC{codecSpecificInfo.data(), codecSpecificInfo.size()}; }
};

struct MetadataPktVideo : MetadataPkt {
  MetadataPktVideo()
      : MetadataPkt(VIDEO_PKT) {}
