...
class GPACDemuxMP4Full : public ModuleS {
  ...
  bool openData() {
    ...
    if((e != GF_OK && e != GF_ISOM_INCOMPLETE_FILE)) {
      m_host->log(Warning,
            format("Error opening fragmented mp4 in progressive mode: %s (missing %s bytes)", gf_error_to_string(e),
                  missingBytes)
                  .c_str());
      return false;
    }
  ...
  bool safeProcessSample() {
    ...
    if(newSampleCount > reader.sampleCount) {
      ...
      m_host->log(Debug,
            format("Found %s new samples (total: %s)", newSampleCount - reader.sampleCount, newSampleCount).c_str());
...

    {
      ...
      m_host->log(Debug,
            format("Found sample #%s(#%s) of length %s , RAP: %s, DTS: %s, CTS: %s", reader.sampleIndex,
                  sample->dataLength, sample->IsRAP, sample->DTS + DTSOffset,
                  sample->DTS + DTSOffset + sample->CTS_Offset)
                  .c_str());
      reader.sampleIndex++;

      auto out = output->allocData<DataRaw>(sample->dataLength);
      memcpy(out->buffer->data().ptr, sample->data, sample->dataLength);
      out->set(PresentationTime{timescaleToClock(
            (int64_t)(sample->DTS + DTSOffset + sample->CTS_Offset), reader.movie->getMediaTimescale(FIRST_TRACK))});
      CueFlags flags{};
      flags.keyframe = true;
      out->set(flags);
